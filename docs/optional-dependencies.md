...
checks, Markdown linting when `markdownlint` is available, Ansible syntax-check
validation when `ansible-playbook` is available, Helm lint/template validation
when `helm` is available, and client-side Kubernetes manifest dry-run validation
when `kubectl` is available. Missing optional tools are reported as skips. `make
validate-dry-run` is available as a descriptive alias for the same non-mutating
checks.
