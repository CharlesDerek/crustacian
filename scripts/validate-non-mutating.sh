...
Helm lint/template rendering, and kubectl client-side dry-run validation when
the related tools and files are available. Missing optional tools or directories
are skipped with a no-op message.
USAGE
		exit 0
		;;
esac
